...
const emailValidator = require('email-validator'); // Para validar sintaxis del email
const dns = require('dns'); // Para verificar si el correo pertenece a Gmai// Ajusta la ruta según sea necesario
 // Para validar sintaxis del email
// Para verificar si el correo pertenece a Gmail
...
router.post('/send-reset-code', (req, res) => {
    ...
    dns.resolveMx(emailDomain, (err, addresses) => {
        ...
        if (!emailDomain.includes("gmail.com")) {
            return res.status(400).json({ message: 'Debe ser un correo de Gmail' });
        }
...
router.post('/verify-reset-code', (req, res) => {
  ...
  connection.query(
      ...
      (err, results) => {
          ...
          const { resetCode, resetCodeExpiration } = results[0];

          if (resetCode !== parseInt(codigo)) {
              return res.status(400).json({ message: 'Código incorrecto' });
          }

          if (new Date() > new Date(resetCodeExpiration)) {
